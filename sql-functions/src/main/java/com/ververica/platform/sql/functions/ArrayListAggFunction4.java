    ...
    return TypeInference.newBuilder()
        .inputTypeStrategy(
            new InputTypeStrategy() {
              ...
              public Optional<List<DataType>> inferInputTypes(
                ...
                // if we need to make sure the input is of a certain type:
                //                if
                // (callContext.getArgumentDataTypes().get(0).getLogicalType().getTypeRoot()
                //                        != LogicalTypeRoot.ARRAY
                //                    && throwOnFailure) {
                //                  return callContext.newValidationError();
                //                }
              ...
              public List<Signature> getExpectedSignatures(FunctionDefinition definition) {
                return null; // to print error messages
              }
            })
        .accumulatorTypeStrategy(
            callContext -> {
              DataType argType = callContext.getArgumentDataTypes().get(0);
              DataType artType2 = DataTypeUtils.toInternalDataType(argType);
              return Optional.of(
                  DataTypes.STRUCTURED(
                      MyAccumulator.class,
                      DataTypes.FIELD("list", ListView.newListViewDataType(artType2)),
        ...
        .outputTypeStrategy(
            callContext -> {
              DataType argType = callContext.getArgumentDataTypes().get(0);
              DataType artType2 = DataTypeUtils.toInternalDataType(argType);
              return Optional.of(DataTypes.ARRAY(artType2).bridgedTo(ArrayData.class));
            })
        .build();
  }
}
